...
// Inner card background based on card type
const typeInnerBg: Record<string, string> = {
  colaborador: 'radial-gradient(circle at 50% 30%, #9e5fb0 0%, #7d4099 50%, #5c2d78 100%)',
  produto: 'radial-gradient(circle at 50% 30%, #4a8eb5 0%, #2d6e8e 50%, #1d4e68 100%)',
}

// Image area background (darker version of type color)
const typeImageBg: Record<string, string> = {
  colaborador: 'linear-gradient(145deg, #3a1a4a, #1a0a2a 50%, #2a1535)',
  produto: 'linear-gradient(145deg, #1a3040, #0a1a25 50%, #152530)',
}
...
export function CardDisplay({ card, tier, compact, selected }: CardDisplayProps) {
  ...
  if (compact) {
    return (
      ...
      >
        <div
          className="flex h-full w-full flex-col overflow-hidden rounded-[4px]"
          style={{ background: typeInnerBg[card.type] }}
        >
          {/* Mini header: type + name */}
          <div className="flex items-center gap-0.5 px-1 pt-[3px]">
            <span
          ...
          {/* Mini separator */}
          <div className="mx-1 mt-[2px] h-px" style={{ backgroundColor: 'rgba(0,0,0,0.25)' }} />

          {/* Mini image area */}
          <div
            className="mx-1 mt-[2px] flex flex-1 items-center justify-center overflow-hidden rounded-[2px]"
            style={{ background: typeImageBg[card.type] }}
          >
            {card.image ? (
          ...
          <div className="flex items-center justify-between px-1 py-[2px]">
            <span className="text-[5px] font-bold leading-none" style={{ color: rarityColor }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-start gap-1.5 px-2.5 pb-1 pt-2">
          <span
            className="shrink-0 rounded-sm px-1.5 py-0.5 text-[7px] font-bold uppercase tracking-wider"
            style={{ backgroundImage: borderGradient, color: '#333' }}
          >
            {card.type === 'colaborador' ? 'Colaborador' : 'Produto'}
          </span>
        ...
        {/* Image area */}
        <div className="mx-2 mt-1.5 overflow-hidden rounded-sm">
          <div
            className="flex items-center justify-center"
            style={{ background: typeImageBg[card.type], aspectRatio: '16/10' }}
          >
            {card.image ? (
              <img src={card.image} alt={card.name} className="h-full w-full object-cover" />
            ) : (
        ...
        {/* Separator */}
        <div
          className="mx-2 mt-1.5 h-px"
          style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.4)', backgroundColor: 'rgba(0,0,0,0.3)' }}
        />

        {/* Description / flavor text */}
        <div className="mx-2 mt-1 flex-1 rounded-sm p-2" style={{ background: 'rgba(255,255,255,0.06)' }}>
        ...
        <div className="flex items-center justify-between px-2.5 py-1">
          <div className="flex items-center gap-1">
            <span className="text-[9px] font-bold leading-none" style={{ color: rarityColor }}>
              {raritySymbols[card.rarity]}
            </span>
